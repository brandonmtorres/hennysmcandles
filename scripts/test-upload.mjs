...
// A genuine 1x1 PNG.
// Which backend is in play. The upload endpoint returns a site-relative path
// on local disk and an absolute bucket URL in production, and both are correct
// — what must hold either way is that the name was generated by us and sits
// under uploads/.
const bucketPublicUrl = (env.S3_PUBLIC_URL ?? '').replace(/\/+$/, '')
const usingBucket = Boolean(env.S3_BUCKET && bucketPublicUrl)

function isStoredImageUrl(url) {
  const pattern = /uploads\/[a-z0-9]+-[a-f0-9]+\.(png|jpg|webp|avif)$/
  if (!pattern.test(url)) return false
  return usingBucket ? url.startsWith(`${bucketPublicUrl}/uploads/`) : url.startsWith('/uploads/')
}
...
// A real image goes through.
{
  ...
  // only answer that matters is whether the image can be fetched back from the
  // URL the shop is about to save — a write that succeeds into a bucket nobody
  // can read renders as a broken product photo.
  if (usingBucket) {
    let served = false
    try {
      const response = await fetch(url)
      served = response.ok && Buffer.from(await response.arrayBuffer()).equals(PNG)
    } catch {
      served = false
    }
    check('served back from the bucket', served, url)
  } else {
